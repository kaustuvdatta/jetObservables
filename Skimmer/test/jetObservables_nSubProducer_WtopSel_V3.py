...
print(jesUncert, systSources,args.year,args.runEra)
'''
...
jetmetCorrector = createJMECorrector(isMC=isMC, applySmearing=False, dataYear='UL'+args.year, jesUncert=jesUncert, runPeriod="" if isMC else args.runEra, applyHEMfix=(True if args.year.startswith('2018') else False) )
fatJetCorrector = createJMECorrector(isMC=isMC, applySmearing=False, dataYear='UL'+args.year, jesUncert=jesUncert, jetType = "AK8PFPuppi", runPeriod="" if isMC else args.runEra, applyHEMfix=(True if args.year.startswith('2018') else False))
...
# our module
from jetObservables.Skimmer.nSubProducer_WtopSel_LeptonicHemTag import nSubProd #_invertedDeltaRWb, #_noDeltaRWb, #try all with >1 and >=1 btags. #_btagCheck
if args.effMap:  modulesToRun.append( nSubProd( sysSource=systSources, leptonSF=LeptonSF[args.year], isMC=isMC, onlyUnc=args.onlyUnc, evtSelection=args.selection, isSigMC=True if args.isSigMC else False, effMapMode=True    )  )
else:   modulesToRun.append( nSubProd( sysSource=systSources, leptonSF=LeptonSF[args.year], isMC=isMC, onlyUnc=args.onlyUnc, evtSelection=args.selection, isSigMC=True if args.isSigMC else False, effMapMode=False    )  )
