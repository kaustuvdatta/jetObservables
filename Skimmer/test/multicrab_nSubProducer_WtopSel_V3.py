...
#fi
    ...
    with open('runPostProc'+options.datasets+options.onlyUnc+'_'+options.year+options.runEra+options.selection+'.sh', 'a') as txtfile:
        cmd = "python "+options.pythonFile+" --sample "+options.datasets+" --selection "+options.selection+" --year "+options.year+" --runEra "+options.runEra+(" --onlyUnc "+options.onlyUnc if options.onlyUnc else "" )+(" --isSigMC" if options.isSigMC else "" )+(" --effMap" if options.effMap else "" )+'\nls\nfi'
        txtfile.write(cmd)
...
def submitJobs( job, inputFiles, unitJobs ):
    ...
    config.Data.splitting = 'FileBased' if not('SingleMuon' in job) else 'LumiBased'#'EventAwareLumiBased'
    config.Data.unitsPerJob = unitJobs if config.Data.splitting=='FileBased' else 50 #10000#5#
    ...
    requestname = 'jetObs_Skim_'+ job.replace('_','').replace('-','')+options.onlyUnc+'_'+options.year +options.selection + '_' +options.version
    ...
    if (isDataFlag ) or (isvNFlag and 'SingleMuon' not in job) :  #'PFNanoAOD-' in inputFiles #('QCD' in job or 'MuEn'in job) and (options.selection.startswith('_W') or options.selection.startswith('_top')):
        s=['v2pt2','v2pt3','v2pt4']
        flag=""
        for x in s:
            if x in inputFiles: 
        ...
        if not(flag==""): 
            if 'APV' in inputFiles and options.year.endswith('VFP') :
                flag='APV'+flag
            config.Data.outputDatasetTag = 'Run'+inputFiles.split('Run')[1].split(flag)[0]+flag+'_jetObsSkim'+options.selection+options.onlyUnc+options.version+('EXT'+job.split('EXT')[1] if 'EXT' in job else '')
        else:
            #e.g., _PFNanoAOD_GT36-
            k='PFNanoAOD-' if 'PFNanoAOD-' in inputFiles else 'PFNano-'
            k='PFNanoAOD_GT36-' if 'PFNanoAOD_GT36-' in inputFiles else 'PFNano-'

            #print(inputFiles.split('Run')[1], inputFiles.split('Run')[1].split(k)[0],k)
            config.Data.outputDatasetTag = 'Run'+inputFiles.split('Run')[1].split(k)[0]+'jetObsSkim'+options.selection+options.onlyUnc+options.version+('EXT'+job.split('EXT')[1] if 'EXT' in job else '')#.outputDatasetTag
    else: 
        k='PFNanoAOD-' if 'PFNanoAOD-' in inputFiles else 'PFNano-'
        config.Data.outputDatasetTag = 'Run'+inputFiles.split('Run')[1].split(k)[0]+'jetObsSkim'+options.selection+options.onlyUnc+options.version+('EXT'+job.split('EXT')[1] if 'EXT' in job else '')#.outputDatasetTag
...
if __name__ == '__main__':
    ...
    parser.add_option(
            "-p", "--pythonFile",
            dest="pythonFile", default="jetObservables_nSubProducer_WtopSel_V3.py",
    ...
    parser.add_option(
            '--effMap',
            action="store_true",
            help="Run skimmer and postprocessor to produce efficiency maps for the provided selection"
            )
    parser.add_option(
            '--isSigMC',
            action="store_true",
    ...
    for sam in dictSamples:
        if sam.startswith( options.datasets ) | options.datasets.startswith('all'):
            selDict = [i for i in checkDict( sam, dictSamples )['selection'] if i==sel]
            if  selDict[0]!= sel:   #
                print("Selection not in dictionary",sel)  
                continue
