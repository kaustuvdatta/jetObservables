...
# our module
from jetObservables.Skimmer.nSubProducer_WtopSel_RunIISummer20UL_CustomTrees_V2 import nSubProd #_invertedDeltaRWb, #_noDeltaRWb, #try all with >1 and >=1 btags.
...
p1=PostProcessor(
    ...
    fwkJobReport = True,
    haddFileName = "jetObservables_"+args.selection+args.year+args.onlyUnc+("sigMC" if args.isSigMC else "")+("bkgMC" if args.isBkgMC else "")+"_nanoskim_V2.root" if args.local else 'jetObservables_nanoskim.root',
    histFileName = "jetObservables_"+args.selection+args.year+args.onlyUnc+("sigMC" if args.isSigMC else "")+("bkgMC" if args.isBkgMC else "")+"_histograms_V2.root" if args.local else 'jetObservables_histograms.root',
