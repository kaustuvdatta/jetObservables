...
if __name__ == '__main__':
    ...
    parser.add_argument("-y", "--year", action='store', choices=['2016_preVFP', '2016', '2017', '2018' ],  default="2017", help="Version" )
    parser.add_argument( '--selection', action="store", help="Event selection", choices=["dijet", "WSel", "topSel"], default="dijet" )
    ...
    for sam in dictSamples:
        if sam.startswith( args.datasets ) | args.datasets.startswith('all'):
            ...
            processingSamples[ sam ] = checkDict( sam, dictSamples )[args.year]['nanoAOD'][0]
    ...
    for isample, jsample  in processingSamples.items():
        if not jsample:
            print('|--------> Sample '+isample+' does not have PFNano sample in datasets dict')
            continue
