# Selection that requires >=1 AK8 jet (w/ pT>200 GeV,y<|1.7|m m_sd>50 GeV), 
...
class nSubProd(Module):
    ...
    def beginJob(self, histFile, histDirName):
        ...
        if not self.onlyUnc:
            ...
            for isel in [ '_noSelnoWeight', '_noSel' ] + allSel:
                ...
                #self.addObject( ROOT.TH1F('nAK4hadjets'+isel,   ';number of had. hem. AK4 jets',   20, 0, 20) )
                #self.addObject( ROOT.TH1F('nAK4lepjets'+isel,   ';number of lept. hem. AK4 jets',   20, 0, 20) )
                self.addObject( ROOT.TH1F('nAK4bjets'+isel,   ';number of b-tagged AK4s',   10, 0, 10) )
                self.addObject( ROOT.TH1F('nAK4bhadjets'+isel,   ';number of b-tagged had. hem. AK4s',   10, 0, 10) )
                self.addObject( ROOT.TH1F('dR_AK8_AK4bhad'+isel,   ';#Delta R( reco AK8, b-tagged AK4 in hadr. hem. )',   100, 0., 2.) )
                #self.addObject( ROOT.TH1F('nAK4blepjets'+isel,   ';number of b-tagged lept. hem. AK4s',   10, 0, 10) )
            ...
            if self.isMC:
                for isel in [ '_noSel' ] + allSel:
                    ...
                    self.addObject( ROOT.TH1F('ngenAK4bhadjets'+isel,   ';number of b-flav. matched had. hem. AK4 genjets',   10, 0, 10) )
                    self.addObject( ROOT.TH1F('dR_genAK8_genAK4bhad'+isel,   ';#Delta R( gen AK8, gen AK4 b-jet in hadr. hem. )',   100, 0, 2.) )
                    #self.addObject( ROOT.TH1F('ngenAK4blepjets'+isel,   ';number of b-flav. matched lept. hem. AK4 genjets',   10, 0, 10) )
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        if not self.isMC: 
            ...
            #tmplistAK4.append('selRecoAK4jetsHadHem_nom') # to store hadronic hem., lead b-tagged AK4's
            #tmplistAK4.append('selRecoAK4jetsLepHem_nom')
        elif self.isMC: 
            for sys in self.sysSource:
                ...
                #self.out.branch('selRecoHadHemDeltaPhi'+sys,"F")
                #self.out.branch('selRecoHadHemDeltaRap'+sys,"F")
                ...
                #self.out.branch('selGenHadHemDeltaPhi'+sys, "F")
                #self.out.branch('selGenHadHemDeltaRap'+sys, "F")
            ...
            #tmplistAK4bjethadHem_reco = ['selRecoAK4jetsHadHem'+sys for sys in self.sysSource ]
            #tmplistAK4bjetleptHem_reco = ['selRecoAK4jetsLepHem'+sys for sys in self.sysSource ]
            #tmplistAK4bjethadHem_gen = [ 'selGenAK4jetsHadHem'+sys for sys in self.sysSource if sys.startswith('_nom') ] 
            #tmplistAK4bjetleptHem_gen = [ 'selGenAK4jetsLepHem'+sys for sys in self.sysSource if sys.startswith('_nom') ] 
    ...
    def getBTagWeight(self, nBTagged=0, jet_SFs=[0]): # Now deprecated, to allow for more than just two btagged jets per event
        ...
        if len(jet_SFs)>2 or nBTagged>2:
            self.n_grt2btags+=1
            if self.n_grt2btags%100==0: print ("Error, only leading and subleading AK4 jets are considered: # of btagged jets cannot exceed 2", nBTagged, self.n_grt2btags)
            return 1
    ...
    def analyze(self, event):
        ...
        if not self.isMC:
            ...
            passGenSel, iGenSel, selGendR_AK8_hadAK4b, selGendPhi_AK8_hadAK4b, selGendRap_AK8_hadAK4b, selGenMuons, selGenAK4bjets, selGenAK4bhadjets, selGenLeptW, selGenJets, selGenMET = self.genSelection(event) 
        passRecoSel, iRecoSel, selRecodR_AK8_hadAK4b, selRecodPhi_AK8_hadAK4b, selRecodRap_AK8_hadAK4b, selRecoMuons, selRecoAK4bjets, selRecoAK4bhadjets, selRecoLeptW, selRecoJets, selRecoMET = self.recoSelection(event) 
        ...
        if not self.isMC:
            if not passRecoSel['_nom']: 
                ...
                self.out.fillBranch( 'recoEventCategory_nom', -1  )
                self.out.fillBranch( 'good_nPVs_nom', 0)
                #self.out.fillBranch( 'nRecoBtags_nom', 0)
                #self.out.fillBranch( 'nRecoHadBtags_nom', 0)
                #self.out.fillBranch( 'nRecoLepBtags_nom', 0)
                #self.out.fillBranch( 'selRecoHadHemDeltaR_nom', 929.)
                #self.out.fillBranch( 'selRecoHadHemDeltaPhi_nom', 929.)
                #self.out.fillBranch( 'selRecoHadHemDeltaRap_nom', 929.)
            ...
            if (not passGenSel) and (not passRecoSel['_nom']): 
                ...
                self.out.fillBranch( 'genEventCategory_nom', -1)
                #self.out.fillBranch( 'nGenBtags_nom', 0)
                #self.out.fillBranch( 'nGenHadBtags_nom', 0)
                #self.out.fillBranch( 'nGenLepBtags_nom', 0)
                #self.out.fillBranch( 'selGenHadHemDeltaR_nom', 929.)
                #self.out.fillBranch( 'selGenHadHemDeltaPhi_nom', 929.)
                #self.out.fillBranch( 'selGenHadHemDeltaRap_nom', 929.)
                #self.out.fillBranch( 'FlagGenLeptHemBjet_nom', 0)
                ...
                if self.isSigMC:# and not self.onlyUnc:
                    ...
                    #self.out.fillBranch( 'btagWeightUp_nom', 0.)
                    #self.out.fillBranch( 'btagWeightDown_nom', 0.)
            ...
            if sysPassFlag==False: 
                for sys in self.sysSource:        
                    ...
                    self.totalRecoWeight=0.
                    self.evtGenWeight=0.
                    self.out.fillBranch( 'recoEventCategory'+sys, -1)
                    ...
                    #self.out.fillBranch( 'nRecoHadBtags'+sys, 0)
                    #self.out.fillBranch( 'nRecoLepBtags'+sys, 0)
                    #self.out.fillBranch( 'selRecoHadHemDeltaR'+sys, 929.)            
                    #self.out.fillBranch( 'selRecoHadHemDeltaPhi'+sys, 929.)            
                    #self.out.fillBranch( 'selRecoHadHemDeltaRap'+sys, 929.)            
                    #self.out.fillBranch( 'FlagRecoLeptHemBjet'+sys, 0)
                    ...
                    self.out.fillBranch( 'genEventCategory'+sys, -1)
                    self.out.fillBranch( 'evtGenWeight'+sys, 0.) 
                    self.out.fillBranch( 'passGenSel'+sys, 0) 
                    #self.out.fillBranch( 'selGenHadHemDeltaR'+sys, 929.)
                    #self.out.fillBranch( 'selGenHadHemDeltaPhi'+sys, 929.)
                    #self.out.fillBranch( 'selGenHadHemDeltaRap'+sys, 929.)                        
        ...
        for sys in self.sysSource:
            ...
            if self.isMC: 
                ...
                if not sys.startswith(('_jes', '_jer')):#self.sysWeightList):
                    ...
                    if self.isSigMC:
                        ...
                        #self.btaggingWeightUp = tempWtbtagup#self.getBTagWeight(nBTagged=len(selRecoAK4bjets),jet_SFs=bTagSFsUp) #tempWtbtagup
                        #self.btaggingWeightDown = tempWtbtagdn#self.getBTagWeight(nBTagged=len(selRecoAK4bjets),jet_SFs=bTagSFsDown) #tempWtbtagdn 
            ...
            if not self.isMC:
                ...
                #self.out.fillBranch( 'selRecoHadHemDeltaPhi'+sys, selRecodPhi_AK8_hadAK4b[sys] if passRecoSel[sys] else 929.)
                #self.out.fillBranch( 'selRecoHadHemDeltaRap'+sys, selRecodRap_AK8_hadAK4b[sys] if passRecoSel[sys] else 929.)
                ...
                #self.out.fillBranch( 'selGenHadHemDeltaR'+sys, selGendR_AK8_hadAK4b if passGenSel or passRecoSel[sys] else 929.)
                #self.out.fillBranch( 'selGenHadHemDeltaPhi'+sys, selGendPhi_AK8_hadAK4b if passGenSel or passRecoSel[sys] else 929.)
                #self.out.fillBranch( 'selGenHadHemDeltaRap'+sys, selGendRap_AK8_hadAK4b if passGenSel or passRecoSel[sys] else 929.)
                ...
                if self.isSigMC and not self.onlyUnc:
                    self.out.fillBranch( 'pdfWeightNom'+sys, self.pdfWeight if passGenSel or passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'pdfWeightAll'+sys, self.pdfWeightAll if passGenSel or passRecoSel[sys] else np.ones((103,),dtype=np.float32))
                    self.out.fillBranch( 'pdfWeightUp'+sys, self.pdfWeightUp if passGenSel or passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'pdfWeightDown'+sys, self.pdfWeightDown if passGenSel or passRecoSel[sys] else 1.)
                    
                    self.out.fillBranch( 'isrWeightUp'+sys, self.isrWeightUp if passGenSel or passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'isrWeightDown'+sys, self.isrWeightDown if passGenSel or passRecoSel[sys] else 1.)
                    
                    self.out.fillBranch( 'fsrWeightUp'+sys, self.fsrWeightUp if passGenSel or passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'fsrWeightDown'+sys, self.fsrWeightDown if passGenSel or passRecoSel[sys] else 1.)
                    
                    self.out.fillBranch( 'puWeightUp'+sys, self.puWeightUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'puWeightDown'+sys, self.puWeightDown if passRecoSel[sys] else 1.)

                    self.out.fillBranch( 'l1prefiringWeightUp'+sys, self.l1PreFireWeightUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'l1prefiringWeightDown'+sys, self.l1PreFireWeightDown if passRecoSel[sys] else 1.)

                    #self.out.fillBranch( 'btagWeightUp'+sys, self.puWeightUp if passRecoSel[sys] else 1.)
                    #self.out.fillBranch( 'btagWeightDown'+sys, self.puWeightDown if passRecoSel[sys] else 1.)

                    self.out.fillBranch( 'leptonWeightAllUp'+sys, self.leptonWeightAllUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightAllDown'+sys, self.leptonWeightAllDown if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightISOUp'+sys, self.leptonWeightISOUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightISODown'+sys, self.leptonWeightISODown if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightIDUp'+sys, self.leptonWeightIDUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightIDDown'+sys, self.leptonWeightIDDown if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightTrigUp'+sys, self.leptonWeightTrigUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightTrigDown'+sys, self.leptonWeightTrigDown if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightRecoEffUp'+sys, self.leptonWeightRecoEffUp if passRecoSel[sys] else 1.)
                    self.out.fillBranch( 'leptonWeightRecoEffDown'+sys, self.leptonWeightRecoEffDown if passRecoSel[sys] else 1.)
    ...
    def recoSelection( self, event, sysUnc=[]  ):
        ...
        ########### Lepton selection ###############
        # only loose selection for electrons, to veto on
        recoElectrons  = [x for x in electrons if x.pt > self.minLooseElectronPt and x.cutBased >= 2 and ((self.range1ElectronEta[0]<abs(x.eta)<self.range1ElectronEta[1]) or (self.range2ElectronEta[0]<abs(x.eta)<self.range2ElectronEta[1]))] 
        
        # applying effectively the isHighPt selection on muons here (since that flag is not explicitly available in our nanos)
        ...
        recoMuons = [x for x in muons if x.pt > self.minTightMuonPt and abs(x.p4().Eta()) < self.maxMuonEta and x.highPtId==2 and x.tkRelIso<0.3] #and x.tightId and abs(x.dxy)<0.2 and abs(x.dz)<0.5 and x.isGlobal and x.isTracker 
        
        recoElectrons.sort(key=lambda x:x.pt, reverse=True)
        
        recoMuons.sort(key=lambda x:x.pt,reverse=True)
        
        # no explicit veto on loose muons in event, just discard event if any >1 muons pass the requirements on our 'tag' muon 
        ...
        iSel = {}
        recodR_AK8_hadAK4b = {}
        recodRap_AK8_hadAK4b = {}
        recodPhi_AK8_hadAK4b = {}
        recoAK4bhadjets = {}
        recoAK4bleptjets = {} 
        for sys in self.sysSource:
            ...
            passSel[sys], iSel[sys], recodR_AK8_hadAK4b[sys], recodPhi_AK8_hadAK4b[sys], recodRap_AK8_hadAK4b[sys], recoAK4bhadjets[sys], recoAK4bleptjets[sys] = self.WtopSelection( False, event, recoMuons, recoElectrons, recoAK4jets, recoAK8jets[sys], MET, sys)#recoAK4bjets
            ...
            ################################################################################################################################
            recoAK4bjets = [x for x in recoAK4bhadjets[sys] if x.btagDeepFlavB > self.minBDisc] + [x for x in recoAK4bleptjets[sys] if x.btagDeepFlavB > self.minBDisc] #these asre not really b-tagged, keeping variable names same for consistency with regular sel.
        ...
        if self.isMC:  
            ...
            #    for i in bTagSFs:
            #        w *= i  
            #    #self.btaggingWeight = self.getBTagWeight(nBTagged=len(recoAK4bjets),jet_SFs=bTagSFs)
            #else:
            #    self.btaggingWeight = 1.#0. ###########<----???
            ####################################################
            ...
            weight =  self.evtGenWeight  * self.puWeight * self.leptonWeight *  self.l1PreFireWeight # * self.btaggingWeight , btagweights['_nom'] # * self.topweight            
        ...
        return passSel, iSel, recodR_AK8_hadAK4b, recodPhi_AK8_hadAK4b, recodRap_AK8_hadAK4b, recoMuons, recoAK4bjets, recoAK4bhadjets, recoleptW, recoAK8jets, MET
    ...
    def genSelection( self, event ):
        ...
        passSel, iSel, gendR_AK8_hadAK4b, gendPhi_AK8_hadAK4b, gendRap_AK8_hadAK4b, genAK4bhadjets, genAK4bleptjets = self.WtopSelection( True, event, genMuons, genElectrons, genAK4jets, genAK8jets, genMET, '' )
        genAK4bjets = [x for x in genAK4bhadjets if abs(x.hadronFlavour)==5] +[ x for x in genAK4bleptjets if abs(x.hadronFlavour)==5]
        #### Weight
        ...
        return passSel, iSel, gendR_AK8_hadAK4b, gendPhi_AK8_hadAK4b, gendRap_AK8_hadAK4b, genMuons, genAK4bjets, genAK4bhadjets, genleptW, genAK8jets, genMET

    #############################################################################
    def WtopSelection( self, isGen, event, muons, electrons, AK4jets,  AK8jets, MET, ptLabel):#AK4bjets,
    
        if (len(muons)==1) and (len(electrons)==0) and (len(AK8jets)>=1) and (len(AK4jets)>3) and MET.Pt()>self.METCutWtop:    # and (len(AK4bjets)>=1)             
            ...
            AK4lepjets = [x for x in AK4jets if self.DrRapPhi(x.p4(),muons[0].p4())<1.6]# if self.DrRapPhi(x.p4(), muons[0].p4() )>0.4 # and and x.btagDeepFlavB > self.minBDisc] 
            AK4lepjets.sort(key=lambda x:x.pt,reverse=True) 

            #if not isGen: 
            if not(len(AK4lepjets)==0):
            ...
            leadJetpT = getattr( AK8jets[0], 'pt'+ptLabel ) #moving this and the line below up here since we cut on the AK8 already here, for the btag eff measurement
            leadJetmass = AK8jets[0].msoftdrop if isGen else getattr( AK8jets[0], 'msoftdrop'+ptLabel ) #AK8jets[0].mass_nom 



            if leadJetmass>self.minleadJetMass and leadJetpT>self.minLeadAK8JetPtW and len(AK4lepjets)>0 and leptW.Pt()>self.minLeptonicWPt and AK8jets[0].p4().DeltaPhi(muons[0].p4())>2. and self.DrRapPhi(AK8jets[0].p4(),AK4lepjets[0].p4())>1.6:
                ...
                #if not isGen: 
                AK4hadjets = [x for x in AK4jets if self.DrRapPhi(x.p4(),AK8jets[0].p4())<1.6]# and x.btagDeepFlavB > self.minBDisc]
                AK4hadjets.sort(key=lambda x:x.pt,reverse=True) 
                AK4hadbjets = [x for x in AK4hadjets]# if self.DrRapPhi(x.p4(),AK8jets[0].p4())<1.6 and x.btagDeepFlavB > self.minBDisc]
                AK4hadbjets.sort(key=lambda x:x.pt,reverse=True) #hackey dummy so that code downstream of this doens't need to be changed much
                #else: 
                ...
                if len(AK4hadjets)>0: 
                    #dummy since no b tagging here
                    dR_leadJet_leadhadAK4 = 929#elf.DrRapPhi(AK4hadbjets[0].p4(),AK8jets[0].p4()) 
                    dPhi_leadJet_leadhadAK4 = 929#AK4hadbjets[0].p4().DeltaPhi(AK8jets[0].p4()) 
                    dRap_leadJet_leadhadAK4 = 929#AK4hadbjets[0].p4().Rapidity() - AK8jets[0].p4().Rapidity()
                else: 
                    dR_leadJet_leadhadAK4 = 929
                    dPhi_leadJet_leadhadAK4 = 929
                    dRap_leadJet_leadhadAK4 = 929
                    return False, None, dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, [], AK4lepbjets

                if self.evtSelection.startswith('_Wtop'):
                    if (len(AK4hadjets)>=1):#leadJetmass>self.minleadJetMass and leadJetpT>self.minLeadAK8JetPtW and
                        ...
                        #if not isGen: print(getattr( AK8jets[0], 'mass'+ptLabel ),AK8jets[0].mass_nom)
                        return True, '_WtopSel', dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets
                    else: 
                        return False, None, dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets

                #elif '_WSel' in self.evtSelection: 
                ...
                #        return True, '_WSel', dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets 
                #    else: 
                #        return False, None, dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets
                ...
                #    if ((leadJetmass > self.minSDMassTop) and (leadJetmass <= self.maxSDMassTop) and (leadJetpT> self.minLeadAK8JetPtTop)) and (len(AK4hadbjets)==1) and (dR_leadJet_leadhadAK4<=0.8): # 
                #        return True, '_topSel', dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets 
                #    else: 
                #        return False, None, dR_leadJet_leadhadAK4, dPhi_leadJet_leadhadAK4, dRap_leadJet_leadhadAK4, AK4hadbjets, AK4lepbjets   
            ...
            else: return False, None, 929, 929, 929, [], AK4lepbjets
        else: return False, None, 929, 929, 929, [], []
