...
class nSubProd(Module):
    ...
    def beginJob(self, histFile, histDirName):
        ...
        if not self.onlyUnc:
            self.addObject( ROOT.TH1F('cutflow_test',   ';Categories',   12, 0, 12) ) #old functionality
            ...
            for isel in [ '_noSelnoWeight', '_noSel' ] + selList:
                ...
                self.addP4Hists( 'LeadingAK8jet', isel )
            ...
            if self.isMC:
                for isel in [ '_noSel' ] + selList :
                    ...
                    self.addP4Hists( 'LeadingAK8genjet', isel )
                ...
                for itype in ( [ 'gen',  'accepgen', 'missgen', 'reco', 'fakereco', 'truereco' ] if self.isMC else [ 'reco' ] ):
                    ...
                    if itype.endswith(('reco', 'gen')):
                        ...
                        for sysUnc in self.sysSource:
                            ...
                            binning = np.array([(i/1000) for i in np.arange(0*1000, 1.*1101)])
                            ...
                            if itype.startswith('truereco') and self.isMC:
                                ...
                                if sysUnc.endswith('nom'):
                                    ...
                                    self.addObject( ROOT.TH1F('resol'+iJ+'_sdmassoverm'+isel, ';AK8 reco m_{SD}/gen jet m', 500, 0, 5) )
                            ...
                            for x, y in self.nSub_labels.items():
                                binning = np.array([(i/1000) for i in np.arange(y[0]*1000, y[1]*1001)])
    ...
    def addP4Hists(self, s, t ):
        ...
        self.addObject( ROOT.TH1F(s+'_mass'+t,';mass (GeV)', 150, 0, 750) )
        if ('reco' in s.lower() or 'AK8' in s) and 'gen' not in s.lower(): self.addObject( ROOT.TH1F(s+'_msoftdrop'+t,';Softdrop mass (GeV)', 100, 0, 500) )
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        #self.out.branch('evtGenWeight', "F" )
        #self.out.branch('puWeight',  "F")
        #self.out.branch( 'puWeightNom', "F" )
        #self.out.branch( 'pdfWeightNom', "F" )
    ...
    def analyze(self, event):
        ...
        for sys in self.sysSource:
            ...
            if sys.startswith(self.sysWeightList):
                ...
                if sys.startswith('_pdfWeight'):
                    ...
                    arr =  getattr( event, 'LHEPdfWeight' ) #convert to a simple numpy array to extract replica weights and thereby the variations
                    arr = root_numpy.tree2array(arr)
                    size = arr.shape[0]#.GetSize() # force loading of array contents for this entry
                    #buf = arr.GetAddress() # still not sure this is always equivalent to `&arr[0]`
                    #buf.SetSize(size)
                    self.pdfWeight = arr[0]
                    ...
                    # however i'm not sure if this is also applicable when eig vect contributions are stored as weights, still in tests seems to give less conservative sigma estimate than doing max(arr[i]-arr[0]) and adding that in as the up/down variation)

                    #npLHEPdfWeight = np.frombuffer(buf, count=size, dtype='float32')
                    #pdfWeight = np.std(npLHEPdfWeight)
                    pdf_std = np.sqrt(np.sum([(arr[i]-arr[0])**2. for i in range(1,103)]))
                    if sys.endswith('Up'):
                ...
                elif sys.endswith('isrWeightDown'): #PSWeight indices corrected and recorrected for Summer20UL Run-II nanov9 produced with latest global tags and CMSSW_10_6_29
            ...
            if self.isMC: self.evtGenWeight=event.genWeight
            ...
            #if not passRecoSel[sys]: 
                #self.fillBranches( 'selRecoJets'+sys, True ) #fill dummy for events not passing
                #self.fillBranches( 'trueRecoJets'+sys, True )
                #self.
            ...
            if passRecoSel[sys]:  #### Detector level dist.
                ...
                for iRJ,ireco in recoJet.items():
                    ...
                    if sys.startswith(self.sysWeightList):
                        ...
                        getattr( self, 'reco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop_nom' ), WEIGHT )
                        getattr( self, 'reco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass_nom' ), WEIGHT )
                    else:
                        getattr( self, 'reco'+iRJ+'_pt'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'pt'+sys ), WEIGHT )
                        getattr( self, 'reco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop'+sys ), WEIGHT )
                        getattr( self, 'reco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass'+sys ), WEIGHT )
                    getattr( self, 'reco'+iRJ+'_eta'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'eta'), WEIGHT )
                    getattr( self, 'reco'+iRJ+'_y'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'rapidity'), WEIGHT )
                ...
                if self.isMC:
                    if not passGenSel:  ##### fakes
                        ...
                        for iRJ,ireco in recoJet.items():
                            ...
                            if sys.startswith(self.sysWeightList):
                                getattr( self, 'fakereco'+iRJ+'_pt'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'pt_nom' ), WEIGHT )
                                getattr( self, 'fakereco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop_nom' ), WEIGHT )
                                getattr( self, 'fakereco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass_nom' ), WEIGHT )
                            else:
                                getattr( self, 'fakereco'+iRJ+'_pt'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'pt'+sys ), WEIGHT )
                                getattr( self, 'fakereco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop'+sys ), WEIGHT )
                                getattr( self, 'fakereco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass'+sys ), WEIGHT )
                            getattr( self, 'fakereco'+iRJ+'_eta'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'eta'), WEIGHT )
                            getattr( self, 'fakereco'+iRJ+'_y'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'rapidity'), WEIGHT )
                            getattr( self, 'fakereco'+iRJ+'_phi'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'phi'), WEIGHT )
                            getattr( self, 'fakereco'+iRJ+'_tau21'+sys+tmpSel ).Fill( ireco['tau21'], WEIGHT )
                            getattr( self, 'fakereco'+iRJ+'_tau32'+sys+tmpSel ).Fill( ireco['tau32'], WEIGHT )
                            if self.runSDVariables:
                        ...
                        #self.fillBranches( 'selGenJets'+sys, True)
                        ...
                        for (iGJ,igen), (iRJ,ireco) in zip( genJet.items(), recoJet.items() ):
                            ...
                            getattr( self, 'gen'+iGJ+'_y'+sys+iGenSel ).Fill( igen['jet'].rapidity, self.totalGenWeight )
                        ...
                        if ( self.DrRapPhi( recoJet['Jet']['jet'].p4(), genJet['Jet']['jet'].p4() ) < 0.4 ):# and ( genJet['Jet2']['jet'].p4().DeltaR( recoJet['Jet2']['jet'].p4() ) > 0.8 ):
                            #iGenSel=''
                            #passGenSel=False
                            deltaRmatch = True
                        ...
                        if deltaRmatch:#( iGenSel==iRecoSel[sys] ) and not self.onlyTrees:
                            ...
                            for (iGJ,igen), (iRJ,ireco) in zip( genJet.items(), recoJet.items() ):
                                ...
                                getattr( self, 'accepgen'+iGJ+'_y'+sys+iGenSel ).Fill( igen['jet'].rapidity, self.totalGenWeight )
                                ...
                                if sys.startswith(self.sysWeightList):
                                    getattr( self, 'truereco'+iRJ+'_pt'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'pt_nom' ), WEIGHT )
                                    getattr( self, 'truereco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop_nom' ), WEIGHT )
                                    getattr( self, 'truereco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass_nom' ), WEIGHT )
                                else:
                                    getattr( self, 'truereco'+iRJ+'_pt'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'pt'+sys ), WEIGHT )
                                    getattr( self, 'truereco'+iRJ+'_msoftdrop'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'msoftdrop'+sys ), WEIGHT )
                                    getattr( self, 'truereco'+iRJ+'_mass'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'mass'+sys ), WEIGHT )
                                getattr( self, 'truereco'+iRJ+'_eta'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'eta'), WEIGHT )
                                getattr( self, 'truereco'+iRJ+'_y'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'rapidity'), WEIGHT )
                                getattr( self, 'truereco'+iRJ+'_phi'+sys+tmpSel ).Fill( getattr(ireco['jet'], 'phi'), WEIGHT )
                                getattr( self, 'truereco'+iRJ+'_tau21'+sys+tmpSel ).Fill( ireco['tau21'], WEIGHT )
                                getattr( self, 'truereco'+iRJ+'_tau32'+sys+tmpSel ).Fill( ireco['tau32'], WEIGHT )
                                if self.runSDVariables:
                                ...
                                if sys.endswith('nom'):
                                    ...
                                    getattr( self, 'resol'+iRJ+'_sdmassoverm'+iGenSel ).Fill( getattr(ireco['jet'], 'msoftdrop')/getattr(igen['jet'], 'mass'))#,WEIGHT)
                                    getattr( self, 'resol'+iRJ+'_mass'+iGenSel ).Fill( getattr(ireco['jet'], 'mass')/getattr(igen['jet'], 'mass'))#, WEIGHT )
                                    try:
                            ...
                            #self.ufoResponse = self.ufoResponse+1   
                        
                        #if not deltaR matched, fill hists for misreconstructed gen jet and fakereco jets
                        else:
                            #self.fillBranches( 'accepGenJets'+sys, True )   #Fill dummy = True  
                            #self.fillBranches( 'trueRecoJets'+sys, True )
                            ...
                            for iGJ,igen in genJet.items():
                                ...
                                getattr( self, 'missgen'+iGJ+'_y'+sys+iGenSel ).Fill( igen['jet'].rapidity, self.totalGenWeight )
        ...
        #self.out.fillBranch( 'evtGenWeight', self.evtGenWeight if self.isMC else -1) #same as quantity right before unless working with sysUncs that affect totalGenWeight (ISR/FSR/PDF)
        #if not ('puWeight' in self.onlyUnc):
        
        #self.out.fillBranch( 'pdfWeightNom', self.pdfWeight if self.isMC else 1)
        #self.out.fillBranch( 'puWeightNom', self.puWeight if self.isMC else 1)
        ...
        #        #self.out.fillBranch( 'isrWeightNom', self.isrWeight if self.isMC else 1)                
        ...
        #        #self.out.fillBranch( 'fsrWeightNom', self.fsrWeight if self.isMC else 1)
        #        self.out.fillBranch( 'fsrWeightDown', self.fsrWeightDown if self.isMC else 1)
        #    if 'puWeight' in self.onlyUnc:
        #        self.out.fillBranch( 'puWeightUp', self.puWeightUp if self.isMC else 1 )
        #        self.out.fillBranch( 'puWeightDown', self.puWeightDown if self.isMC else 1 )
    ...
    def recoSelection( self, event, sysUnc=[] ):
        ...
        recoMuons = [x for x in muons if x.pt > self.minLooseMuonPt and abs(x.p4().Eta()) < self.maxMuonEta and x.pfIsoId>=2 and x.mediumId and abs(x.dxy)<0.2 and abs(x.dz)<0.5 and x.miniPFRelIso_all<0.20] 
        ...
        #print ("in reco sel", event.event, recoMuons, recoElectrons, AK8jets)
        ...
        if self.isMC:
            ...
            #if not self.onlyUnc: getattr( self, 'PUweight' ).Fill( event.puWeight )
        ...
        if not self.onlyUnc:
            ...
            if not len(recoAK8jets['_nom'])==0:
                getattr( self, 'LeadingAK8jet_pt_noSelnoWeight' ).Fill( recoAK8jets['_nom'][0].pt_nom )
                getattr( self, 'LeadingAK8jet_eta_noSelnoWeight' ).Fill( recoAK8jets['_nom'][0].eta )
                getattr( self, 'LeadingAK8jet_y_noSelnoWeight' ).Fill( recoAK8jets['_nom'][0].rapidity )
                getattr( self, 'LeadingAK8jet_phi_noSelnoWeight' ).Fill( recoAK8jets['_nom'][0].phi )
                getattr( self, 'LeadingAK8jet_mass_noSelnoWeight' ).Fill( recoAK8jets['_nom'][0].mass_nom )
            ...
            if not len(recoAK8jets['_nom'])==0:
                getattr( self, 'LeadingAK8jet_pt_noSel' ).Fill( recoAK8jets['_nom'][0].pt_nom, weight )
                getattr( self, 'LeadingAK8jet_eta_noSel' ).Fill( recoAK8jets['_nom'][0].eta, weight )
                getattr( self, 'LeadingAK8jet_y_noSel' ).Fill( recoAK8jets['_nom'][0].rapidity, weight )
                getattr( self, 'LeadingAK8jet_phi_noSel' ).Fill( recoAK8jets['_nom'][0].phi, weight )
                getattr( self, 'LeadingAK8jet_mass_noSel' ).Fill( recoAK8jets['_nom'][0].mass_nom, weight )
            ...
            if ('_nom' in iSel.keys()) and passSel['_nom'] and iSel['_nom']:
                ...
                for imuon in recoMuons:
                    ...
                    getattr( self, 'muons_y'+iSel['_nom'] ).Fill( imuon.p4().Rapidity(), reweight )
                ...
                for iele in recoElectrons:
                    ...
                    getattr( self, 'eles_y'+iSel['_nom'] ).Fill( iele.p4().Rapidity(), reweight )
                ...
                for ijet in recoAK8jets['_nom']:
                    ...
                    getattr( self, 'AK8jets_y'+iSel['_nom'] ).Fill( ijet.rapidity, reweight )
                    getattr( self, 'AK8jets_phi'+iSel['_nom'] ).Fill( ijet.phi, reweight )
                    getattr( self, 'AK8jets_mass'+iSel['_nom'] ).Fill( ijet.mass_nom, reweight )
                if not len(recoAK8jets['_nom'])==0:
                    getattr( self, 'LeadingAK8jet_pt'+iSel['_nom'] ).Fill( recoAK8jets['_nom'][0].pt_nom, reweight )
                    getattr( self, 'LeadingAK8jet_eta'+iSel['_nom'] ).Fill( recoAK8jets['_nom'][0].eta, reweight )
                    getattr( self, 'LeadingAK8jet_y'+iSel['_nom'] ).Fill( recoAK8jets['_nom'][0].rapidity, reweight )
                    getattr( self, 'LeadingAK8jet_phi'+iSel['_nom'] ).Fill( recoAK8jets['_nom'][0].phi, reweight )
                    getattr( self, 'LeadingAK8jet_mass'+iSel['_nom'] ).Fill( recoAK8jets['_nom'][0].mass_nom, reweight )
    ...
    def genSelection( self, event ):
        ...
        if not self.onlyUnc:
            ...
            if not len(genAK8jets)==0:
                getattr( self, 'LeadingAK8genjet_pt_noSel' ).Fill( genAK8jets[0].pt, weight )
                getattr( self, 'LeadingAK8genjet_eta_noSel' ).Fill( genAK8jets[0].eta, weight )
                getattr( self, 'LeadingAK8genjet_y_noSel' ).Fill( genAK8jets[0].rapidity, weight )
                getattr( self, 'LeadingAK8genjet_phi_noSel' ).Fill( genAK8jets[0].phi, weight )
                getattr( self, 'LeadingAK8genjet_mass_noSel' ).Fill( genAK8jets[0].mass, weight )
            ...
            if passSel and iSel:
                ...
                if not len(genAK8jets)==0:
                    getattr( self, 'LeadingAK8genjet_pt'+iSel ).Fill( genAK8jets[0].pt, weight )
                    getattr( self, 'LeadingAK8genjet_eta'+iSel ).Fill( genAK8jets[0].eta, weight )
                    getattr( self, 'LeadingAK8genjet_y'+iSel ).Fill( genAK8jets[0].rapidity, weight )
                    getattr( self, 'LeadingAK8genjet_phi'+iSel ).Fill( genAK8jets[0].phi, weight )
                    getattr( self, 'LeadingAK8genjet_mass'+iSel ).Fill( genAK8jets[0].mass, weight )
    ...
    #############################################################################
    def fillBranches( self, jetLabel, jetInfo=[], dummy=False ): 
        ...
        if not dummy:
            ...
            if 'reco' in jetLabel.lower(): self.out.fillBranch(jetLabel+"_msoftdrop", [ iJ['jet'].msoftdrop_nom for i,iJ in jetInfo.items() ] )
            ...
            self.out.fillBranch(jetLabel+"_y", [ iJ['jet'].rapidity for i,iJ in jetInfo.items()  ] )
            ...
            #fill dummies
            self.out.fillBranch( 'n'+jetLabel, -1.)
            self.out.fillBranch(jetLabel+"_mass", dummyFill)# for i,iJ in jetInfo.items() ] )
            if 'reco' in jetLabel.lower(): self.out.fillBranch(jetLabel+"_msoftdrop", dummyFill)#[ iJ['jet'].msoftdrop_nom for i,iJ in jetInfo.items() ] )
            self.out.fillBranch(jetLabel+"_pt", dummyFill)#[ iJ['jet'].pt for i,iJ in jetInfo.items()  ] )#, 'pt'+jetLabel.split('Jets')[1] ) for i,iJ in jetInfo.items()  ] )
            self.out.fillBranch(jetLabel+"_eta", dummyFill)#[ iJ['jet'].eta for i,iJ in jetInfo.items()  ] )
            self.out.fillBranch(jetLabel+"_y", dummyFill)#[ iJ['jet'].rapidity for i,iJ in jetInfo.items()  ] )
            self.out.fillBranch(jetLabel+"_phi", dummyFill)#[ iJ['jet'].phi for i,iJ in jetInfo.items()  ] )
            self.out.fillBranch(jetLabel+"_tau21", dummyFill) #[ iJ['tau21'] for i,iJ in jetInfo.items()  ] )
            self.out.fillBranch(jetLabel+"_tau32", dummyFill)# [ iJ['tau32'] for i,iJ in jetInfo.items()  ] )
            if self.runSDVariables:
                self.out.fillBranch(jetLabel+"_SD_tau21", dummyFill)#[ iJ['sdtau21'] for i,iJ in jetInfo.items()  ] )
                self.out.fillBranch(jetLabel+"_SD_tau32", dummyFill)#[ iJ['sdtau32'] for i,iJ in jetInfo.items()  ] )

            for tauN in range(1, self.maxTau+1):
                self.out.fillBranch(jetLabel+"_tau_0p5_"+str(tauN),  dummyFill)#[ iJ['0p5'+str(tauN)] for i,iJ in jetInfo.items() ] )
                self.out.fillBranch(jetLabel+"_tau_1_"+str(tauN),  dummyFill)#[ iJ['1'+str(tauN)] for i,iJ in jetInfo.items() ] )
                self.out.fillBranch(jetLabel+"_tau_2_"+str(tauN),  dummyFill)#[ iJ['2'+str(tauN)] for i,iJ in jetInfo.items() ] )
                if self.runSDVariables:
                    self.out.fillBranch(jetLabel+"SD_tau_0p5_"+str(tauN),  dummyFill)#[ iJ['sd0p5'+str(tauN)] for i,iJ in jetInfo.items() ] )
                    self.out.fillBranch(jetLabel+"SD_tau_1_"+str(tauN),  dummyFill)#[ iJ['sd1'+str(tauN)] for i,iJ in jetInfo.items() ] )
                    self.out.fillBranch(jetLabel+"SD_tau_2_"+str(tauN),  dummyFill)#[ iJ['sd2'+str(tauN)] for i,iJ in jetInfo.items() ] )
