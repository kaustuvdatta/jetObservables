...
def plotTriggerEfficiency( inFileSample, sample, triggerDenom, triggerPass, triggerScale, name, xlabel, xmin, xmax, rebin, log):
    ...
    DenomOnly = inFileSample.Get( sample+'_'+triggerDenom+'_pt' )
    #DenomOnly.Rebin(rebin)
    Denom = DenomOnly.Clone()
    PassingOnly = inFileSample.Get( sample+'_'+triggerPass+'_pt' )
    #PassingOnly.Rebin(rebin)
    PassingOnly.Scale( triggerScale )
    Passing = PassingOnly.Clone()
    Efficiency = ROOT.TGraphAsymmErrors( Passing, Denom, 'cp'  )
    Efficiency = ROOT.TEfficiency( Passing, Denom )
    ...
    Efficiency.SetLineColor(ROOT.kBlue-4)
    ...
    can1 = ROOT.TCanvas('c1', 'c1',  10, 10, 750, 500 )
    Efficiency.SetMarkerStyle(8)
    Efficiency.SetMarkerColor(ROOT.kGray)
    ...
    PassingOnly.SetLineColor(ROOT.kBlue-5)
    ...
    axis.SetTitleColor(ROOT.kBlue-5)
    axis.SetTitleSize(0.06)
    axis.SetLabelSize(0.05)
    axis.SetTitleFont(42)
    axis.SetLabelFont(42)
    axis.SetLineColor(ROOT.kBlue-5)
    axis.SetLabelColor(ROOT.kBlue-5)
...
###################################################################
def coffeaToRoot(inputFile):
    """docstring for coffeaToRoot"""

    result = load(inputFile)
    outputfile = uproot.recreate(inputFile.replace('coffea', 'root'))
...
if __name__ == '__main__':
    ...
    Samples = {}
    #Samples[ 'SingleMuon_Run2017C' ] = 'hists_samples_2017_SingleMuon_Run2017C.coffea'
    Samples[ 'JetHT_Run2017B' ] = 'hists_samples_2017_JetHT_Run2017B.coffea'
    ...
    for isam, samFile in processingSamples.items():
        coffeaToRoot( samFile )
        inputFile = ROOT.TFile( samFile.replace('coffea', 'root') )
        for i in Plots:
            for q, it in enumerate(triggerList):
                plotTriggerEfficiency( inputFile, isam, 'triggerAK8PFJet140', 'trigger'+it, triggerScales[q], i[0], i[1], i[2], i[3], i[4], i[5] )
