...
def runTUnfold(
    ...
    ###########################################################################################
        if year.startswith('all'):
            ...
            if not process.startswith('MCSelfClosure'): altSignalHistos = loadHistograms( tmp2SigFiles, ivar, sel, sysUnc=[], isMC=True, respOnly=False, lumi=lumi, year=year, process=process, variables=variables,outputFolder='../Results' )
            
            if not process.endswith('Closure'): sysSignalHistos = loadHistograms( sysSigFiles, ivar, sel, sysUnc=sysUncert, respOnly=False, isMC=True, lumi=lumi, year=year, process=process, variables=variables,outputFolder='../Results' )
            ...
            bkgHistos = loadHistograms( bkgFiles, ivar, sel, sysUnc=[], lumi=lumi, year=year, process=process, variables=variables,outputFolder='../Results' ) if process.startswith('data') else {}

            #print ("Alt histos:", bkgHistos)
            
            ####### Fix fake and true reco ###nothing to fix here anymore :) 
            print ("All signal histo keys:", signalHistos.keys())
            ...
            if not process.startswith('MCSelfClosure'):
                print("All alt. signal MC histo keys:",altSignalHistos)
            ...
            if sel.startswith('_dijet'): 
                if process.startswith("MC"):
                    ...
                    genBin = variables[ivar]['bins']
                    dataHistostrue = loadHistograms( dataFile, ivar, sel, isMC= False, sysUnc=[], respOnly=False, lumi=lumi, year=year, process=process, variables=variables,outputFolder='../Results' )
                ######## Cross check: plotting data vs all MC Scaled
                print ('|------> Cross check: plotting data vs all MC')
                #print(dataHistostrue,dataFile)
                allHistos = {}
            ...
            if sel.startswith('_dijet'):
                ...
                print ("SF genBin and recobin, respectively:",scaleFactor,scaleFactorGenBin)
        ...
        for ih in tmpList:
            ...
            if isMC:
                ...
                if sel.startswith('_dijet') and process.startswith('data'):
                    ...
                    for it in checkDict( 'JetHT', dictSamples )[year]['triggerList']:
                        ...
                        tmpdataHistos[ it ] = itempFile.Get( ih.replace( sel, '_'+it+sel ) )
            ...
            if not noRebin:
                if len(variables[var]['bins'])==1:
                    genBin = 100#variables[var]['bins'][0]
                    recoBin = 50#variables[var]['bins'][0]/2
                else:
                ...
                if not ih.startswith('resp'):
                    ...
                    if len(variables[var]['bins'])==1: 
                        allHistos[isam+'_'+ih].Rebin2D( genBin,recoBin )
                    else:
                        ...
                        #axis = allHistos[isam+'_'+ih].GetXaxis()
                        #old_gen_bin_edges = [axis.GetBinLowEdge(i) for i  in range(1,axis.GetNbins()+2)]
                        #new_gen_bin_edges = genBin
                        #new_bin_edge_pairs = get_bin_edge_pairs(new_bins=new_gen_bin_edges,old_reco_bin_edges=old_gen_bin_edges)
                        ...
                        #for biny in range(allHistos[isam+'_'+ih].GetNbinsY()+2):
                        #    by=allHistos[isam+'_'+ih].GetYaxis().GetBinCenter(biny)
                        #    for binx in range(allHistos[isam+'_'+ih].GetNbinsX()+2):
                        #        if not(binx==0 or biny==0):# or binx==allHistos[isam+'_'+ih].GetNbinsX()+1 or biny==allHistos[isam+'_'+ih].GetNbinsY()+1): 
                        #            continue
                        #        bx=allHistos[isam+'_'+ih].GetXaxis().GetBinCenter(binx)
                        #        jbin = allHistos[isam+'_'+ih].GetBin(binx,biny)

                        #        if allHistos[isam+'_'+ih].IsBinUnderflow(jbin):

                        #            j=0 
                        #            for i in range(tmpHisto.GetNbinsX()+2):
                        #                if (bx>tmpHisto.GetXaxis().GetBinLowEdge(i) and bx<tmpHisto.GetXaxis().GetBinLowEdge(i+1)):
                        #                    tmpHisto.SetBinContent(i,j, tmpHisto.GetBinContent(i,j)+allHistos[isam+'_'+ih].GetBinContent(jbin))
                        #                    tmpHisto.SetBinError(i,j, ROOT.TMath.Sqrt(tmpHisto.GetBinError(i,j)**2+allHistos[isam+'_'+ih].GetBinError(jbin)**2))

                        #tmpHisto.Sumw2()
                        #tmpHisto.SetDirectory(0)
                        #allHistos[isam+'_'+ih] = copy.deepcopy(tmpHisto.Clone())
                        
                        #### fancy way to create variable binning TH2D
                        tmpHisto = ROOT.TH2F( h_temp.GetName()+"_Rebin", h_temp.GetName()+"_Rebin", len(genBin)-1, array( 'd', genBin), len(recoBin)-1, array( 'd', recoBin) )
                        #tmpHisto = make_rebinned_2d_hist(h_temp.Clone(), new_bin_edge_pairs,True)
                        tmpHisto.Sumw2()                        
                        #tmpArrayContent = np.zeros((len(genBin), len(recoBin)))
                        #tmpArrayError = np.zeros((len(genBin), len(recoBin)))


                        for biny in range( 0, h_temp.GetNbinsY()+2 ):
                            by = h_temp.GetYaxis().GetBinCenter( biny )
                            for binx in range( 0, h_temp.GetNbinsX()+2 ):
                                bx = h_temp.GetXaxis().GetBinCenter(binx)
                                if not(binx==0 or biny==0 or binx==h_temp.GetNbinsX()+1 or biny==h_temp.GetNbinsY()+1):
                                    ...
                                    for iX in range( len(genBin)-1 ):
                                        for iY in range( len(recoBin)-1 ):
                                            if (bx<genBin[iX+1] and bx>genBin[iX]) and (by<recoBin[iY+1] and by>recoBin[iY]):
                                                jbin = h_temp.GetBin(binx,biny)
                                                #tmpArrayContent[iX][iY] = tmpArrayContent[iX][iY] + h_temp.GetBinContent( jbin )
                                                #tmpArrayError[iX][iY] = tmpArrayError[iX][iY] + ROOT.TMath.Power( h_temp.GetBinError( jbin ), 2 )
                                                tmpHisto.SetBinContent( tmpHisto.GetBin(iX+1,iY+1), tmpHisto.GetBinContent(iX+1,iY+1)+h_temp.GetBinContent( jbin )) #tmpArrayContent[binx-1][biny-1] )
                                                tmpHisto.SetBinError( tmpHisto.GetBin(iX+1,iY+1), np.sqrt((tmpHisto.GetBinError(iX+1,iY+1))**2.+(h_temp.GetBinError( jbin ))**2.))#tmpArrayError[binx-1][biny-1] ) )
                                else:
                                    if not(binx==0 or biny==0):# or binx==h_temp.GetNbinsX()+1 or biny==h_temp.GetNbinsY()+1): 
                                        continue
                                    #bx=h_temp.GetXaxis().GetBinCenter(binx)
                                    ...
                                    if h_temp.IsBinUnderflow(jbin):

                                        j=0 
                                        for i in range(tmpHisto.GetNbinsX()+2):
                                            if (bx>tmpHisto.GetXaxis().GetBinLowEdge(i) and bx<tmpHisto.GetXaxis().GetBinLowEdge(i+1)):
                                                tmpHisto.SetBinContent(i,j, tmpHisto.GetBinContent(i,j)+h_temp.GetBinContent(jbin))
                                                tmpHisto.SetBinError(i,j, np.sqrt((tmpHisto.GetBinError(i,j))**2.+(h_temp.GetBinError(jbin))**2.))
                        #tmpHisto.Sumw2()
                        tmpHisto.SetDirectory(0)
                        allHistos[isam+'_'+ih] = copy.deepcopy(tmpHisto.Clone())
...
def getAndPlotPurity(h_resp_rebinned,gen_bins,var,outputDir='../Results/'):
    ...
    makePSplot_simple(purity=array('d',p_list),stability=array('d',s_list),
                      dictHistos=dictHistos,var=var,outputDir=outputFolder,bins=gen_bins)
    return 1