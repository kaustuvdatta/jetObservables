...
import time
import re
from datasets_dijetSel_RunIISummer20UL_T3 import dictSamples, checkDict
...
class nSubBasis_unfoldingHistoProd_Dijets(processor.ProcessorABC):
    ...
    def __init__(self, sampleName, sysSource=[],year='2017', era='', isMC=True, isSigMC=True, 
                 onlyUnc='', wtUnc=False, sampleDict=dictSamples,test=False, sysUnc=False):#isaltSigMC=False,
        self.test=test
        self.year = year
        self.isMC = isMC
        ...
        self.dict_variables_toUnfold = {
                    ...
                    "_tau_1_1": np.array([(i/1000) for i in np.arange(0.*1000, 1.*1001)]),
                    "_tau_1_2": np.array([(i/1000) for i in np.arange(0.*1000, 0.7*1001)]),
                    "_tau_1_3": np.array([(i/1000) for i in np.arange(0.*1000, 0.5*1001)]),
                    ...
                    "_tau21_WTA": np.array([(i/1000) for i in np.arange(0.*1000, 1.6*1001)]),#for WTA-kT for comparison
                    "_tau32_WTA": np.array([(i/1000) for i in np.arange(0.*1000, 1.6*1001)]),#for WTA-kT for comparison
                    #"_tau21_exkT": np.array([(i/2000) for i in np.arange(0.*2000, 2.*2001)]),#for excl.-kT and E-scheme as per basis
                    #"_tau32_exkT": np.array([(i/2000) for i in np.arange(0.*2000, 2.*2001)]),#for excl.-kT and E-scheme as per basis
        ...
        self.wtSources=['_puWeight','_isrWeight','_fsrWeight','_pdfWeight'] if self.wtUnc else [] 
    ...
    def process(self, events):
        ...
        print (f'Processing histogram production for {self.sampleName} in {self.year}')
        if self.isSigMC and self.wtUnc: print(f'including the following systematic weight variations {self.wtSources}') 
        if self.isSigMC and self.onlyUnc: print(f'including the following systematic variations {self.onlyUnc}') 
        ...
        for sys in self.sysSource:
            ...
            if self.isMC and self.isSigMC:# or self.isaltSigMC:
                ...
                trueRecoMask = (events[f'trueRecoJets{s}_pt']>0.) & (selRecoMask)
                fakeRecoMask = (events[f'trueRecoJets{s}_pt']<0.) & (selRecoMask)
                selGenMask = events[f'evtGenWeight_nom'] !=0.
                accepGenMask = (events[f'accepGenJets{s}_pt']>0.) & (selGenMask)
                missGenMask = (events[f'accepGenJets{s}_pt']<0.) & (selGenMask)
                ...
                selRecoMasks = OrderedDict()#masking now trigger dependent to create different histos for data events passing certain triggers or not
                
                for itrigger, itrValues in self.triggerTable.items():                    
                    selRecoMasks[itrigger]=(events[f'totalRecoWeight{sys}']!=0.) & (events[f'selRecoJets{sys}_pt']>itrValues[self.year][0]) & (events[f'selRecoJets{sys}_pt']  < itrValues[self.year][1])
            ...
            for isel in self.selList:
                ...
                for k in listOfOutputHistosNames:
                    key=k
                    histoName=output[key]# in output.items()[listOfOutputHistos]:
                    #if (sys in key): print ("Check:",sys,key)
                    ############### Safety checks ##################
                    if not(sys in key) and not('resol' in key): 
                        #print (sys,key)
                        #print (f'{key} doesn\'t correspond to {sys}')
                        continue

                    if (not self.isMC) and not('AK8PF' in key):
                        #print (key, "this key is in a data histo dict but doesn't have a trig label")
                        continue
                    ...
                    if not('_tau' in key):
                        ...
                        temp='WTA' if 'WTA' in key else 'tau' #hack, :(, to fix tau21_WTA being histogrammed incorrectly
                        whichnSub = [var for var in self.dict_variables_toUnfold.keys() if (var in key and temp in var)]#'_tau'+key.split(sys)[0].split('_tau')[1]
                    ...
                    if not sys.startswith(self.sysWeightList):
                        ...
                        #if sys.endswith('nom'): print (totalRecoWeight)
                    else:
                        s='_nom'
                        #print (sys)
                        if 'pu' in sys:
                            totalGenWeight = events[f'evtGenWeight{s}'] 
                            totalRecoWeight = totalGenWeight*events[f'{sys.split("_")[1]}{s}']
                            #if 'tau_1_1' in key: print (sys,totalRecoWeight)

                        elif sys.startswith(('_isr','_fsr','_pdf')):#,'_pdf''):
                            totalGenWeight = events[f'evtGenWeight{s}']*events[f'{sys.split("_")[1]}{s}']
                            totalRecoWeight = totalGenWeight*events[f'puWeightNom{s}']

                    if (key.lower().startswith(('reco','good'))):
                        if 'nPV' in key:
                            #print (varToFill,key,events[f'good_nPVs{s}'][selRecoMasks[whichTrig]])
                    ...
                    if self.isMC and (not varToFill.startswith(tuple(self.reco_only_labels))):
                        ...
                        if (key.lower().startswith('genjet')):
                            output[key].fill(events[f'selGenJets_nom{varToFill}'][selGenMask],weight=totalGenWeight[selGenMask])#=self._listofHistograms(histoName) #hist.Hist("Events", hist.Cat("branch", branch), hist.Bin("value", branch, 100, 0, 1000))    

                        elif (key.lower().startswith('accepgenjet')):# and not key.startswith(('accep','miss')):
                            output[key].fill(events[f'accepGenJets{s}{varToFill}'][accepGenMask],weight=totalGenWeight[accepGenMask])#=self._listofHistograms(histoName) #hist.Hist("Events", hist.Cat("branch", branch), hist.Bin("value", branch, 100, 0, 1000))    

                        elif (key.lower().startswith('missgenjet')):
                            output[key].fill(events[f'selGenJets_nom{varToFill}'][missGenMask],weight=totalGenWeight[missGenMask])#=self._listofHistograms(histoName) #hist.Hist("Events", hist.Cat("branch", branch), hist.Bin("value", branch, 100, 0, 1000))    

                        elif ('resp' in key.lower()):
                            #print (key)
                            output[key].fill(gen=events[f'accepGenJets{s}{varToFill}'][accepGenMask],reco=events[f'trueRecoJets{s}{varToFill}'][trueRecoMask],
                                             weight=totalRecoWeight[trueRecoMask])
                            ...
                            zeroMask=(events[f'accepGenJets{s}{varToFill}']!=0.)&(accepGenMask)#include accepgen mask in zeromask tp not have to 'double-mask' #
                            #zeroMaskreco=(events[f'accepGenJets{s}{varToFill}']!=0.)&(events[f'trueRecoJets{sys}_pt']>0.) & (events[f'totalRecoWeight{sys}']!=0.)

                            response = events[f'trueRecoJets{s}{varToFill}'][zeroMask]/events[f'accepGenJets{s}{varToFill}'][zeroMask]
                            #events[f'accepGenJets{s}{varToFill}'][accepGenMask][events[f'accepGenJets{s}{varToFill}'][accepGenMask]==0]='-999'
    ...
    def postprocessHistograms_MC_Split(myProcessor,sampleIdentifier='qcd_ht', y='2017', sampleDict_PFNano=OrderedDict(),sampleDict_local=OrderedDict(),
                                 isSigMC=True,isMC=True,wtUnc=True,sysUnc=False,onlyUnc='', outputdir='processorTests/', ext='_nomWts',splitchunks=5,nWorkers=2):
        # need to modify for data
        ...
        for sample in sampleDict_local.keys():
            ...
            for year in yl:
                if not( sampleIdentifier in sample.lower()):
                    continue
                tstart=time.time()
                #print (sample)
                ...
                my_processor=myProcessor(sampleName=sample, sampleDict=sampleDict_PFNano,
                                         ...
                                         wtUnc=wtUnc,sysUnc=sysUnc,onlyUnc=onlyUnc )
                if splitchunks>0:
                    fl=[]
                    n_tosplit = []
                    c=0
                ...
                for i in ifls:
                    ...
                    events = uproot.concatenate(stringfnames+':Events', my_processor._branchesToRead, step_size="1024 MB", library='ak', num_workers=10)
                    processed_events=my_processor.process(events)
                    if c==0: hists=processed_events
                    else: hists=accumulate([hists,processed_events])
                    
                    
                    
                    #print (processed_events)
                    print ("Events proccesed, now writing output files")
                    if not sysUnc: 
                        fn = f'{outputdir}/jetObservables_histograms_{sample.split("_Tune")[0].split("_")[0]+sample.split("_Tune")[0].split("_")[1]}_UL{year}{ext}_{c}.root'
                        fnstem = f'{outputdir}/jetObservables_histograms_{sample.split("_Tune")[0].split("_")[0]+sample.split("_Tune")[0].split("_")[1]}_UL{year}{ext}'
                    else: 
                        fn = f'{outputdir}/{sampleDict_local[sample][year]["skimmerHisto"].split(".root")[0]}{ext}_{c}.root'
                        fnstem = f'{outputdir}/{sampleDict_local[sample][year]["skimmerHisto"].split(".root")[0]}{ext}'
                    if splitchunks==0: fn=f'{fnstem}.root'
                    print (fn)#,fnstem)
                ...
                if splitchunks>0:
                    with uproot.recreate(f'{fnstem}.root') as fout_hadd:
                        ...
                        print (f'Done with accumulated creating output file:{fnstem}.root')
                        fout_hadd.close()
    ...
    def postprocess(self, accumulator):
        #for histo in accumulator.items():
        #    print (histo)
        #    uproot.write(accumulator, "output.root", histo)
        pass
        '''
    ...
    def buildDictOfHistograms(self):
        ...
        #print ( f'Booking histograms for {self.sampleName} in year {self.year} and era {self.era}')#, using branches: {self._branchesToRead}')
    ...
    def getBranchesToRead(self, dirname, year, kinematic_labels, 
        ...
        #print (f"Reading {dirname[0].split('kadatta/jetObservables')[1].split('/')[1]} in UL{year})#,\n will write read and correctly masked arrays (for MC) to {dirname[0].split('kadatta/jetObservables')[1].split('/')[1]+'_UL'+year+'.parquet'} unless asked not to save explicitly in args")
        #if self.sysUnc and not onlyUnc: self.sysSource = ['_nom'] + [ isys+i for i in [ 'Up', 'Down' ] for isys in self.sysSource if not isys.endswith('nom') ]
        
        #elif self.sysUnc and onlyUnc: 
        #el
        if self.wtUnc: 
        ...
        for sys in self.sysSource:
            ...
            if self.isMC and (sys.endswith('nom') or self.sysUnc):
                ...
                #reco_list.append("evtRecoWeight"+sys)
        ...
        elif (not self.isMC): branchesToRead=reco_list#gen_list+
