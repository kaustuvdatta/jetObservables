...
def runTUnfold(
                ...
                year='2016_preVFP',runMLU=False, sysSigFiles=[], varSigFiles=[], outputFolder='../Plots_June23/',
                version='_June23', mainMC='TTToSemiLeptonic', altMC='TTJets',verbose=False
              ):
    
    
    colors = [ 2, 4,  9, 8, 28, 30, 42, 13, 12, 40, 46, 3, 24, 26, 41, 45, 48, 49, 37, 38, 33, 17]

    if mainMC.startswith('TTToSemiLeptonic'):
        signalLabelBegin = 'TTToSemiLeptonic'
        signalLabel = 'TTToSemiLeptonic'

    elif mainMC.startswith('TTJets'):
        signalLabelBegin = 'TTJets'
        signalLabel = 'TTJets'

    if altMC.startswith('TTJets'):
        altSignalLabelBegin = 'TTJets'
        altSignalLabel = 'TTJets'
    
    elif altMC.startswith('TTToSemiLeptonic'):
        altSignalLabelBegin = 'TTToSemiLeptonic'
        altSignalLabel = 'TTToSemiLeptonic'
    ...
    ###########################################################################################
        if year.startswith('all'):
            ...
            if not process.endswith('Closure'): 
                ...
                # SM bkg. processes
                bkgHistos = loadHistograms( bkgFiles, ivar, sel, isMC=True, respOnly=False, lumi=lumi, year=year, process=process, variables=variables,outputFolder=outputFolder)
                #signal variation samples (for mtop, tune, CR, etc. uncertainties)
                varSignalHistos = loadHistograms( varSigFiles, ivar, sel, sysUnc=[], respOnly=True, isMC=True, lumi=lumi, year=year, process=process, variables=variables,outputFolder=outputFolder)
